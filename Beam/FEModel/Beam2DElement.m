    ...
    methods
        ...
        function [rhs_aux, G, x_eta] = get_eq_gp(obj, F, Ua, i_gp, for_Ua0)
            ...
            if for_Ua0
                ...
                G(1:2) = G(1:2) / r_bar;
                J0_ = norm(G(1:2));
                rhs_aux(ind_aux_G3) = rhs_aux(ind_aux_G3) / J0_;
        ...
        function Q = get_Q(obj, G, i_gp)
            eta = obj.loc_gp{obj.n_gp}(i_gp);
            shpfcn = obj.shape_function{obj.number_of_nodes}(eta);
            der_shpfcn = obj.der_shape_function{obj.number_of_nodes}(eta) / obj.J0(i_gp);
            
            n_cols = size(G, 2);
            Q = zeros(3, 3*obj.number_of_nodes, n_cols);
            for j = 1:n_cols
                G_hat = [0 -G(3, j) G(2, j); G(3, j) 0 -G(1, j)];
                for k = 1:obj.number_of_nodes
                    Q(1:3, (1:3) + (k-1)*3, j) = der_shpfcn(k) * eye(3);
                    Q(1:2, (1:3) + (k-1)*3, j) = Q(1:2, (1:3) + (k-1)*3, j) + shpfcn(k) * G_hat;
                end
            end
        end
        ...
        function QT = get_QT(obj, G, i_gp)
            ...
            for j = 1:n_cols
                ...
                for k = 1:obj.number_of_nodes
                    ...
                    QT((1:3) + (k-1)*3, 1:2, j) = QT((1:3) + (k-1)*3, 1:2, j) + shpfcn(k) * G_hat';
                end
            end
        end
        
        function R = get_R(obj, G, i_gp)
            n_cols = size(G, 2);
            defo = G;
            for j = 1:n_cols
                defo(:, j) = defo(:, j) - obj.G0(:, i_gp);
            end
            ...
            Q = zeros(3, 3 * obj.number_of_nodes);
            for j = 1:n_cols
                G_hat = [0 -G(3, j) G(2, j); G(3, j) 0 -G(1, j)];
                for k = 1:obj.number_of_nodes
                    Q(1:3, (1:3) + (k-1)*3) = der_shpfcn(k) * eye(3);
                    Q(1:2, (1:3) + (k-1)*3) = Q(1:2, (1:3) + (k-1)*3) + shpfcn(k) * G_hat;
                end
                R(:, j) = R(:, j) + Q' * (obj.weight_gp{obj.n_gp}(i_gp) * obj.J0(i_gp) * stress(:, j));
            end
        end
        ...
        function [R, Ra] = assemble(obj, U, Ua)
            ...
            for i = 1:obj.n_gp
                ...
                %                 Q = obj.get_Q(G, i);
                %                 for j = 1:n_cols
                %                     R(:, j) = R(:, j) + Q(:,:,j)' * (obj.weight_gp{obj.n_gp}(i) * obj.J0(i) * stress(:, j));
                %                 end
                
                QT =  obj.get_QT(G, i);
                if n_cols > 1
                    R = R + squeeze(sum(QT .* reshape(obj.weight_gp{obj.n_gp}(i) * obj.J0(i) * stress, 1, 3, n_cols), 2));
                else
                    R = R + QT * obj.weight_gp{obj.n_gp}(i) * obj.J0(i) * stress;
                end
            end

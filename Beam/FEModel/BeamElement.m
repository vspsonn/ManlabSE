    ...
    methods
        ...
        function [rhs_aux, G, x_eta] = get_eq_gp(obj, F, Ua, i_gp, for_Ua0)
            ...
            if for_Ua0
                ...
                G(1:3) = G(1:3) / r_bar;
                J0_ = norm(G(1:3));
                rhs_aux(ind_aux_G46:ind_aux_G46+2) = rhs_aux(ind_aux_G46:ind_aux_G46+2) / J0_;
        ...
        function Q = get_Q(obj, G, i_gp)
            ...
            n_cols = size(G, 2);
            Q = zeros(6, 6*obj.number_of_nodes, n_cols);
            for j = 1:n_cols
                sG_theta = skew(G(4:6, j));
                sG_u = skew(G(1:3, j));
                for k = 1:obj.number_of_nodes
                    Q(1:3, (1:3) + (k-1)*6, j) = der_shpfcn(k) * eye(3) + shpfcn(k) * sG_theta;
                    Q(1:3, (4:6) + (k-1)*6, j) = shpfcn(k) * sG_u;
                    Q(4:6, (4:6) + (k-1)*6, j) = Q(1:3, (1:3) + (k-1)*6, j);
                end
            end
        end

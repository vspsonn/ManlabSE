classdef StraightBeamFlexDef < ComponentDef
    properties
        n_nodes_per_beam = 2
    ...
    methods
        function obj = StraightBeamFlexDef(f_root, f_tip, beam_props, n_elem)
        ...
        function [obj, U, ind_nodes] = mesh(obj, ind_nodes)
            ...
            d = SO3_Log(SO3_composition(SO3_inverse(obj.f_root), obj.f_tip));
            ...
            for i = 1:obj.n_elem
                for j = 1:(obj.n_nodes_per_beam-2)
                    ...
                    U((1:4) + ind_nodes * 4) = SO3_composition(obj.f_root, SO3_Exp(alpha * d));
                    
                    ind_nodes = ind_nodes + 1;
                    list_nodes(j+1) = ind_nodes;
                end
                if i == obj.n_elem
                    U((1:4) + ind_nodes * 4) = obj.f_tip;
                else
                    alpha = i / obj.n_elem;
                    U((1:4) + ind_nodes * 4) = SO3_composition(obj.f_root, SO3_Exp(alpha * d));
                end
                ...
                obj.list_elem{i} = BeamFlex2Element(list_nodes, obj.beam_props);
                obj.list_elem{i}.n_gp = 2;
                
                list_nodes(1) = list_nodes(obj.n_nodes_per_beam);
            end
            
            for i = 1:n_nodes
                obj.list_elem{obj.n_elem + i} = UnitQuatConstraint(ind_nodes0 + i, 3);
            end
